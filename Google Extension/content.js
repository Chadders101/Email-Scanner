async function getColourFromServer(emailData, id) {
  try {
...
function processLinkData(links, numberOfLinks) {
...
if (window.location.hostname.includes("yahoo.com")) {
    ...
    function saveClassificationToStorage(id, colour) {
      if (colour !== '#000000') { // Check if the colour is not black
        ...
        console.log(`Classification for ${id} is black and will not be stored.`);
      }
    }
    ...
    async function fetchEmailContentAndAnalyse(href, notificationBox) {
      ...
      if (result.hasOwnProperty(id)) {
        ...
        return; // Exit if we already have a colour stored
      }
    ...
    // Extract links from the email content and process them
    const links = Array.from(doc.links).map(link => link.href);
    //console.log(id, doc)
    
    // Calculate the number of links
    const num_links = Array.from(new DOMParser().parseFromString(contentHtml, 'text/html').links).length;
    console.log(num_links)

    const linkDataList = processLinkData(links);

      // Check if the ratio_digits_url indicates an error
      // this value is either 0 or 0.28817834742296156 when an error occurs.
      //this catches server outages or too many requests overloading the server
      if (linkDataList.ratio_digits_url === 0 || linkDataList.ratio_digits_url === 0.28817834742296156 ) {
        console.log(`Error ratio detected for ${id}, assigning black.`);
        notificationBox.style.backgroundColor = '#000000'; // Set the notification box to black
        return; // Exit the function early
      }

    linkDataList.nb_links = num_links
    //console.log(id, linkDataList)

    // Compile the cleaned data into an object for the server
    const emailData = {
      linksData: linkDataList,
      //content: textContent,
      //title: title,
      //sender: sender
    };
    ...
    const colourResponse = await getColourFromServer(emailData, id);
    ...
    // set to black so classification is not stored
    notificationBox.style.backgroundColor = '#000000'; 
  }
}
